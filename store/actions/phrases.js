...
export const handleLoadPhrases =
  ({ word }) =>
...
export const handlePostPhrase =
  ({ word, textInput, isPublic }) =>
  async (dispatch, getState) => {
    const { userProfile: authedUser } = getState();
    const phrase = {
      word,
...
export const handleRemovePhrase =
  ({ phraseId }) =>
  async (dispatch) => {
    dispatch({
      type: REMOVE_PHRASE,
...
export const handleTogglePhraseLike =
  ({ phraseId, authedUserId, hasLiked, likes }) =>
  async (dispatch) => {
    dispatch({
...
export const handleTogglePhraseVisibility =
  ({ phraseId, isPublic }) =>
  async (dispatch) => {
    dispatch({
